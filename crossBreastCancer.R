...
##### This file Runs a cross validation 5 times cross validation on the Breast Cancer Data Set ####
setwd("~/Dropbox/Code/DPmixturemodel/SBC")
rm(list =ls())
source('import.R')

### Load the data
data(vanDeVijver, package="seventyGeneData")
## Load the Pheno Data for VIJVER
pheno.vijver <- read.csv('/home/bit/ashar/ExpressionSets/ONE_VIEW/breastcancer/VIJVERpheno.csv')
## Load the 70 gene siganture
source('loadVIJVERsignature.R')
loadVIJVERsignature()

########## Preparing the Data Matrix ######################################
Y.full <- t(impute(exprs(vanDeVijver)))

######## Defining the Pheno type ###########
time.pre <- pheno.vijver$Follow_up_time_or_metastasis 
censoring.pre <- pheno.vijver$event_metastasis
### Define the Poor vs Good discrimination from VIJVER
...
cluster.parallel <- makeCluster(2, type= "FORK")
library(doParallel)
...
# recov.logrank.vijver <- c(0)
# pred.logrank.vijver <- c(0)
# 
...
#### Define the folds
folds <- createFolds(c.vijver, k = 2, list = TRUE, returnTrain = FALSE)
 
#### The actual cross-validation loop contains around 200 lines of code
### Begins here #####
results <- foreach(i = 1:1, .export = ls(),.packages = list.p) %dopar%{
test.index <- folds[[i]]
...
#### Use VIJVER's classification to come up with separability and c-Index##
#### Using VIJVER training and Testing Data Sets ##########################
Y.vijver.train <- Y.pre.train[,colnames(Y.pre.train) %in% vijver.signature]
Y.vijver.test <- Y.pre.test[,colnames(Y.pre.test) %in% vijver.signature]

###
recov.logrank.vijver<- unlist(survdiff(smod ~ c.true))$chisq
pred.logrank.vijver <-  unlist(survdiff(smod.new ~ c.true.new))$chisq
...
for ( q in 1:2){
  ...
  reg.cox <- cv.glmnet(x = Y.vijver.train[ind,], y = smod[ind], family = "cox", maxit = 10000000)
  linear.vv.recovery[ind] <- predict(object =reg.cox, newx = Y.vijver.train[ind,], s= "lambda.min")
  linear.vv.prediction[ind.new] <- predict(object =reg.cox, newx = Y.vijver.test[ind.new,], s= "lambda.min")
  }
...
fitControl <- trainControl(method = "repeatedcv", number = 5,repeats = 5, allowParallel = FALSE)
### Tune the parameter k
knnFit <- train(x = Y.vijver.train, y = label.train, method =  "knn", trControl = fitControl, tuneLength = 5)
knnPredict <- predict(knnFit,newdata = Y.vijver.test)
...
for ( q in 1:2){
  ...
  reg.cox <- cv.glmnet(x = Y.vijver.train[ind,], y = smod[ind], family = "cox", maxit = 10000000)
  linear.vv.kk.prediction[ind.new] <- predict(object =reg.cox, newx = Y.vijver.test[ind.new,], s= "lambda.min")
}
...
#### Use SBC signature ##
rel <- loadSBCvijversignature(Y.pre.train, time, censoring)
...
d <- dist(as.matrix(Y))
hc <- hclust(d)
label.train <- as.factor(cutree(hc, k = F))
...
list('recov.logrank.vijver'= recov.logrank.vijver,
       'pred.logrank.vijver' = pred.logrank.vijver,
